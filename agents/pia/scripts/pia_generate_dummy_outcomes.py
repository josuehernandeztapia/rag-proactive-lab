...
Usage:
    ...
    python agents/pia/scripts/pia_generate_dummy_outcomes.py \
        --contracts data/processed/pia/synthetic_contracts.csv \
        --states data/processed/pia/synthetic_driver_states.csv \
        --summary-out reports/pia_plan_summary.csv

El script usa `data/processed/pia/protection_contracts_dummy.csv` junto con los
estados sintéticos de cartera para evaluar el equilibrio de protección, registrar
el mejor escenario viable y recalcular las agregaciones (features y plan summary)."""
...
DEFAULT_SUMMARY_PATH = get_path('reports', 'pia_plan_summary')
DEFAULT_FEATURES_PATH = get_path('data', 'processed', 'hase', 'outcomes_features')
DEFAULT_CONTRACTS_PATH = get_path('data', 'processed', 'pia', 'synthetic_contracts')
DEFAULT_STATES_PATH = get_path('data', 'processed', 'pia', 'synthetic_states')
...
def load_contracts(path: Path) -> List[ProtectionContract]:
    ...
    if not path.exists():
        raise FileNotFoundError(f"No se encontró el CSV de contratos dummy: {_rel(path)}")
        ...
        payload.update(
            {
                ...
                "driving_hours_30d": _to_float(state.get("driving_hours_30d"), 0.0),
                "idling_hours_30d": _to_float(state.get("idling_hours_30d"), 0.0),
                ...
                "seatbelt_off_rate_30d": _to_float(state.get("seatbelt_off_rate_30d"), 0.0),
                "high_speed_ratio_30d": _to_float(state.get("high_speed_ratio_30d"), 0.0),
                "idle_hours_ratio_30d": _to_float(state.get("idle_hours_ratio_30d"), 0.0),
                "after_hours_distance_km_30d": _to_float(state.get("after_hours_distance_km_30d"), 0.0),
                "after_hours_driving_hours_30d": _to_float(state.get("after_hours_driving_hours_30d"), 0.0),
                "after_hours_stop_hours_30d": _to_float(state.get("after_hours_stop_hours_30d"), 0.0),
                "after_hours_ratio_30d": _to_float(state.get("after_hours_ratio_30d"), 0.0),
                "idle_pressure_30d": _to_float(state.get("idle_pressure_30d"), 0.0),
                "high_speed_pressure_30d": _to_float(state.get("high_speed_pressure_30d"), 0.0),
                "seatbelt_pressure_30d": _to_float(state.get("seatbelt_pressure_30d"), 0.0),
                "telemetry_health_score": _to_float(state.get("telemetry_health_score"), 0.0),
                "safety_score": _to_float(state.get("safety_score"), 0.0),
                "safety_alert": _to_int(state.get("safety_alert"), 0),
    ...
    decision.details.update(
        {
            ...
            "seatbelt_off_rate_30d": payload.get("seatbelt_off_rate_30d", 0.0),
            "high_speed_ratio_30d": payload.get("high_speed_ratio_30d", 0.0),
            "idle_hours_ratio_30d": payload.get("idle_hours_ratio_30d", 0.0),
            "after_hours_ratio_30d": payload.get("after_hours_ratio_30d", 0.0),
            "telemetry_health_score": payload.get("telemetry_health_score", 0.0),
            "safety_score": payload.get("safety_score", 0.0),
            "safety_alert": payload.get("safety_alert", 0),
...
def run_aggregation(log_path: Path, features_out: Path, summary_out: Path, windows: List[int]) -> None:
    try:
        ...
        print(f"No outcome log found at {_rel(log_path)}, skipping aggregation", file=sys.stderr)
        return
    if df.empty:
    ...
    print(f"Características agregadas guardadas en {_rel(features_out)} ({len(aggregated)} placas)")
    write_metadata(
        features_out,
        script=__file__,
        inputs=[log_path],
        extra={"rows": int(len(aggregated)), "windows": windows},
    )
    ...
    if summary_out:
        ...
        print(f"Resumen por plan guardado en {_rel(summary_out)}")
        write_metadata(
            summary_out,
            script=__file__,
            inputs=[log_path, features_out],
            extra={"rows": int(len(plan_summary))},
        )
...
def maybe_reset_log(log_path: Path) -> None:
    if log_path.exists():
        log_path.unlink()
        print(f"Log reiniciado: {_rel(log_path)}")


def main() -> int:
    args = parse_args()
    if args.reset_log:
        maybe_reset_log(Path(args.log))
    ...
    total_viable = 0
    for index, contract in iterate_contracts(contracts_path):
    ...
    if not args.skip_aggregate:
        run_aggregation(log_path, features_out, summary_out, args.windows)
    else:
