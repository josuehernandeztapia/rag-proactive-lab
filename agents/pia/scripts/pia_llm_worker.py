...
Modo de uso:
    python3 agents/pia/scripts/pia_llm_worker.py \
        --features data/processed/hase/pia_outcomes_features.csv \
        --interval 60 \
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Ruta del CSV que se monitorea (default data/processed/hase/pia_outcomes_features.csv)",
    )
    parser.add_argument(
        "--interval",
        type=int,
...
def run_notifier(extra_args: str) -> int:
    notifier_path = PROJECT_ROOT / "agents" / "pia" / "scripts" / "pia_llm_notifier.py"
...
def worker(features: Path, interval: int, extra_args: str, oneshot: bool) -> None:
    ...
    if not features.exists():
        print(f"[pia_llm_worker] El archivo {_rel(features)} no existe todavía; esperando...", flush=True)
    while True:
        if features.exists():
            current_mtime = features.stat().st_mtime
            if last_mtime is None or current_mtime > last_mtime:
                print(f"[pia_llm_worker] Cambio detectado en {_rel(features)}; disparando pia_llm_notifier", flush=True)
            ...
            print(f"[pia_llm_worker] {_rel(features)} aún no existe...", flush=True)
            last_mtime = None
