...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--contact-default-phone", default="", help="Teléfono por defecto si no hay mapeo")
    parser.add_argument("--contacts-csv", type=Path, help="CSV con columnas para personalizar saludo/teléfono (placa, contacto, nombre)")
    parser.add_argument("--contact-column", default="contact", help="Nombre de la columna con el teléfono en contacts-csv")
    parser.add_argument("--contact-name-column", default="nombre", help="Columna con el nombre a saludar (opcional)")
...
def load_contacts(
    ...
    if contact_column not in df.columns:
        print(f"[guardian:notifier] Columna {contact_column} no encontrada en {path}; se ignora mapeo", file=sys.stderr)
        return {}
    has_name = name_column in df.columns
    mapping: Dict[str, Dict[str, str]] = {}
    for _, row in df.iterrows():
        placa = str(row.get("placa") or "").strip()
        contact = str(row.get(contact_column) or "").strip()
        name = str(row.get(name_column) or "").strip() if has_name else ""
        if placa:
            mapping[placa] = {"contact": contact, "name": name}
    return mapping
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    contacts_map = load_contacts(
        resolve_path(ROOT, args.contacts_csv) if args.contacts_csv else None,
        args.contact_column,
        args.contact_name_column,
    )
    ...
    for _, row in selected.iterrows():
        ...
        if enqueue_whatsapp_message and not args.dry_run:
            try:
                quick_replies = []
                keyword = msg_bundle.get("handoff_keyword")
                if keyword:
                    quick_replies.append(str(keyword))
