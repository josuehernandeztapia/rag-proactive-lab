...
import db_cases
from s3_storage import presign_upload as s3_presign_upload, public_url as s3_public_url, available as s3_available
...
def confirm_upload(case_id: str, body: ConfirmBody):
    ...
    ok = db_cases.add_attachment(body.case_id, body.kind, url, ocr=ocr, meta={"object_key": body.object_key})
    if not ok:
        raise HTTPException(status_code=500, detail="No se pudo guardar adjunto en DB")
    return {"ok": True, "url": url, "ocr": ocr}
