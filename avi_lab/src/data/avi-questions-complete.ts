...
import { AVIQuestionEnhanced, AVICategory, AVIConfig, RiskImpact } from './avi-types';

// Re-export types for easier imports
export { AVIQuestionEnhanced, AVICategory, AVIConfig, RiskImpact } from './avi-types';
...
export const AVI_CONFIG: AVIConfig = {
  total_questions: 55,
  ...
  questions_by_category: {
    'BASIC_INFO': ALL_AVI_QUESTIONS.filter(q => q.category === AVICategory.BASIC_INFO).length,
    'DAILY_OPERATION': ALL_AVI_QUESTIONS.filter(q => q.category === AVICategory.DAILY_OPERATION).length,
    'OPERATIONAL_COSTS': ALL_AVI_QUESTIONS.filter(q => q.category === AVICategory.OPERATIONAL_COSTS).length,
    'BUSINESS_STRUCTURE': ALL_AVI_QUESTIONS.filter(q => q.category === AVICategory.BUSINESS_STRUCTURE).length,
    'ASSETS_PATRIMONY': ALL_AVI_QUESTIONS.filter(q => q.category === AVICategory.ASSETS_PATRIMONY).length,
    'CREDIT_HISTORY': ALL_AVI_QUESTIONS.filter(q => q.category === AVICategory.CREDIT_HISTORY).length,
    'PAYMENT_INTENTION': ALL_AVI_QUESTIONS.filter(q => q.category === AVICategory.PAYMENT_INTENTION).length,
    'RISK_EVALUATION': ALL_AVI_QUESTIONS.filter(q => q.category === AVICategory.RISK_EVALUATION).length
  },
...
export function getQuestionsByCategory(category: AVICategory): AVIQuestionEnhanced[] {
  return ALL_AVI_QUESTIONS.filter(q => q.category === category);
}

export function getQuestionsByWeight(minWeight: number): AVIQuestionEnhanced[] {
  return ALL_AVI_QUESTIONS.filter(q => q.weight >= minWeight);
}

export function getQuestionsByStressLevel(minStressLevel: number): AVIQuestionEnhanced[] {
  return ALL_AVI_QUESTIONS.filter(q => q.stressLevel >= minStressLevel);
}

export function getCriticalQuestions(): AVIQuestionEnhanced[] {
  return ALL_AVI_QUESTIONS.filter(q => q.weight >= 9);
}

export function getHighStressQuestions(): AVIQuestionEnhanced[] {
  return ALL_AVI_QUESTIONS.filter(q => q.stressLevel >= 4);
}

export function getQuestionById(id: string): AVIQuestionEnhanced | undefined {
  return ALL_AVI_QUESTIONS.find(q => q.id === id);
}

export function getRandomQuestions(count: number): AVIQuestionEnhanced[] {
  const shuffled = [...ALL_AVI_QUESTIONS].sort(() => 0.5 - Math.random());
  return shuffled.slice(0, count);
}

export function getQuestionsForDemo(): AVIQuestionEnhanced[] {
  return [
    getQuestionById('nombre_completo')!,
    getQuestionById('ingresos_promedio_diarios')!,
    getQuestionById('gastos_mordidas_cuotas')!,
    getQuestionById('creditos_anteriores')!,
    getQuestionById('motivacion_credito')!
  ].filter(Boolean);
}