...
PLAYBOOKS_FILE = os.getenv("PLAYBOOKS_FILE", os.path.join(os.path.dirname(__file__), "playbooks.json"))
        ...
        try:
            with pool.connection() as conn:
                ...
                with conn.cursor() as cur:
                    cur.execute(
                        """
                        INSERT INTO events (id, ts, kind, endpoint, question, answer, classification, signals, meta, contact_address, conversation_id, channel)
                        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                        """,
                        (
                            event_id, ts, kind, endpoint, question, answer, classification, Json(signals), Json(meta), str(contact_addr) if contact_addr else None, conversation_id, channel
                        ),
                    )
                    if sources:
                        for s in sources:
...
def update_case(contact: str, patch: dict) -> dict:
    ...
    for k, v in (patch or {}).items():
        if k in ('categories', 'required', 'provided', 'attachments'):
            # merge lists unique
            cur = c.get(k) or []
            if isinstance(v, list):
                cur = list(dict.fromkeys(cur + v))
                c[k] = cur
        elif v is not None:
            c[k] = v
